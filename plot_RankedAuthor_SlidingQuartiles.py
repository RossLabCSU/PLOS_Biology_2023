...
def plot_medians(df, xcat, ylabel, fig_label):
    ...
    q1_vals = [val for i, val in enumerate(df['Value']) if df['Category'][i] == '25th Percentile']
    q3_vals = [val for i, val in enumerate(df['Value']) if df['Category'][i] == '75th Percentile']
    xvals = [xval for i, xval in enumerate(df[xcat]) if df['Category'][i] == '75th Percentile']
    ...
    plt.savefig(fig_label + '_' + xcat + '_SelfReferencingRate_SlidingQuartiles.tif', bbox_inches='tight', dpi=600)
    plt.close()
