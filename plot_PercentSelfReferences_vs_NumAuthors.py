...
def plot_medians_alljournals(df, fig_label):
    ...
    plt.savefig(fig_label + '_PercentSelfReferences_vs_NumAuthors_Medians.tif', bbox_inches='tight', dpi=600)
    plt.close()
...
def subplots(df, fig_label):
    ...
    for index, journal in enumerate(journal_order + ['All Publications']):
        ...
        xvals = [xval for i, xval in enumerate(temp_df['Number of Authors']) if cat_vals[i] == '75th Percentile']
    ...
    plt.savefig(fig_label + '_PercentSelfReferences_vs_NumAuthors_JournalSubplots.tif', bbox_inches ='tight', dpi=600)
    plt.close()  
