...
def plot_medians_alljournals(df, fig_label):
    ...
    plt.savefig(fig_label + '_PercentSelfReferences_vs_NumReferences_Medians.tif', bbox_inches='tight', dpi=600, pil_kwargs={'compression':'tiff_lzw'})
    plt.close()
...
def subplots(df, fig_label):
    ...
    for index, journal in enumerate(journal_order + ['All Publications']):
        ...
        xvals = [xval for i, xval in enumerate(temp_df['Total References']) if cat_vals[i] == '3rd Quartile (Q3)']
