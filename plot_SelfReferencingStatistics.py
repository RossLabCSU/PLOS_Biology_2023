...
def centraltendency_barplot(df, fig_panel):
    ...
    plt.savefig(fig_panel + '_AllJournals_Mean_and_Median_SelfReferencePercentages.tif', bbox_inches='tight', dpi=600)
    plt.close()
...
def percentiles_boxplot(percentiles_df, scatter_df, fig_panel):
    ...
    fig = plt.gcf()
    fig.set_size_inches(5,4)
    plt.savefig(fig_panel + '_AllJournals_Percentiles_SelfReferencePercentages.tif', bbox_inches='tight', dpi=600)
    plt.close()
